        ...
        for (int i = 0; i < Movers.Count; i++)
        {
            for (int j = 0; j < Movers.Count; j++)
            {
                if (i != j)
                {
                    ...
                    Vector2 attractedMover = Movers[j].attract(Movers[i].body,-1);
            ...
            //Vector2 dir = Vector3.Distance(mousePos, Movers[i].body.transform.position);
            Movers[i].body.AddForce(Movers[i].attract(mousePos,-2));
...
public class Mover2_8
{
    ...
    public Vector2 attract(Vector3 l, float mult)
    {
        Vector2 force = body.position - l;
        float distance = Vector3.Distance(body.position, l);
