...
{
    Mover_10 mover;
    [SerializeField] float Baseacceleration;  // Start is called before the first frame update
    void Start()
    {
        mover = new Mover_10();
    ...
    void Update()
    {
        ...
        float acceleration = Baseacceleration*(3/Vector3.Distance(mover.location, mousePos));
        Debug.Log(Vector3.Distance(mover.location, mousePos));
        mover.acceleration = mover.multiplyVector(dir.normalized, acceleration);
        mover.Update();
    }
